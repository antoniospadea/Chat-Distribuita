...
class Oracle:
    ...
    def peer_registration(self, message, address):
        ...
        if message.startswith('-r'):
            ...
            if message.isalnum():
                # Controlla se il nickname è già presente nella lista dei Peer registrati e in caso invia un
                # messaggio di errore
                if message not in self.peer_list.keys():
                    # Voglio anche inviare 3 peer random al peer che si è registrato cosi che possa inserirli nella
                    # sua lista dei vicini Devo passare sia il nickname (che è la chiave del dizionario) che
                    # l'indirizzo e la porta (che è il valore del dizionario) Per fare questo devo prima creare una
                    # lista di chiavi e poi estrarre tre chiavi random Questo va fatto se però la lista dei peer
                    # registrati è maggiore di tre
                    if len(self.peer_list) > 3:
                        ...
                        # Invia i vicini al Peer con davanti il tag -d (done) cosi che sappia che la registrazione è
                        # andata a buon fine
    ...
    def send_communication(self, tag, nick, address):
        ...
        if tag == '-r':
            # Invia il messaggio agli altri Oracle con davanti il tag -r (registration) cosi che sappiano che è una
            # registrazione Creiamo un dizionario con chiave il nickname e valore l'indirizzo del peer
